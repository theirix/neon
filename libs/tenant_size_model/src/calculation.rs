use crate::{SegmentMethod, SizeResult, StorageModel};
...
struct SizeAlternatives {
    // cheapest alternative if previous is available.
    incremental: SegmentSize,

    // cheapest alternative if previous node is not available
    non_incremental: Option<SegmentSize>,
}

impl StorageModel {
    pub fn calculate(&self) -> Vec<SizeResult> {
        ...
        for root in roots {
            if let Some(selected) = self.size_here(root, &child_list).non_incremental {
                self.fill_selected_sizes(&selected, &mut result);
            } else {
    ...
    fn fill_selected_sizes(&self, selected: &SegmentSize, result: &mut Vec<SizeResult>) {
        result[selected.seg_id] = SizeResult {
    ...
    fn size_here(&self, seg_id: usize, child_list: &Vec<Vec<usize>>) -> SizeAlternatives {
        ...
        let snapshot_here = if !seg.needed {
        ...
        // If the previous is not available, what's the cheapest method involving
        ...
        if let Some(snapshot_later) = snapshot_later {
            ...
            if let Some(prev) = &cheapest_non_incremental {
                if snapshot_later.accum_size <= prev.accum_size {
        ...
        // And what's the cheapest method, if the previous is available?
