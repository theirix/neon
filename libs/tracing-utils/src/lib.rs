...
///
/// We try to follow the conventions for the environment variabes as specified in
...
///   are supported, asthey are handled by the `opentelemetry-otlp` crate.
...
/// Like `init_tracnig`, but creates a separate tokio Runtime for the tracing
/// tasks.
...
fn init_tracing_internal(service_name: String) -> opentelemetry::sdk::trace::Tracer {
    ...
    // exporter uses the URL from OTEL_EXPORTER_OTLP_ENDPOINT as is, without
    // appending "/v1/traces".
    //
    // See https://github.com/open-telemetry/opentelemetry-rust/pull/950
    //
    // Work around that by checking OTEL_EXPORTER_OTLP_ENDPOINT and appending
    // the "/v1/traces" path ourselves. If the bug is fixed in a later
    // version, we can remove this code. But if don't remember to remove this,
    // it won't do any harm either, as the crate will ignore the
    // OTEL_EXPORTER_OTLP_ENDPOINT setting if the endpoint is set directly
    // with `with_endpoint`.
