...
use pageserver::tenant::layer_map::{LayerMap, LayerMapLayer};
...
impl LayerMapLayer for DummyDelta {
...
impl LayerMapLayer for DummyImage {
    fn get_key_range(&self) -> Range<Key> {
        self.key_range.clone()
    }
...
fn build_layer_map(filename_dump: PathBuf) -> LayerMap<dyn LayerMapLayer> {
    let mut layer_map = LayerMap::<dyn LayerMapLayer>::default();
...
fn uniform_query_pattern(layer_map: &LayerMap<dyn LayerMapLayer>) -> Vec<(Key, Lsn)> {
...
fn bench_sequential(c: &mut Criterion) {
    let mut layer_map: LayerMap<dyn LayerMapLayer> = LayerMap::default();
