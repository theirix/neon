...
impl Timeline {
    ...
    pub async fn calculate_logical_size(&self, up_to_lsn: Lsn) -> anyhow::Result<u64> {
        info!(
...
impl Timeline {
    ...
    async fn gc_timeline(
        ...
        for doomed_layer in layers_to_remove {
            ...
            layer_paths_to_delete.push(doomed_layer.filename());
    ...
    ///
    /// If there is a download error, the function retries indefinitel, using an exponential back-off.
    ...
    #[instrument(skip_all, fields(tenant_id=%self.tenant_id, timeline_id=%self.timeline_id, file=%remote_layer.filename().display()))]
